...
class Pants : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        add_photo?.setOnClickListener {
            ...
            val mBuilder = AlertDialog.Builder(this@Pants)
        ...
        pants_image?.setOnTouchListener(View.OnTouchListener { view, event ->
    ...
    fun deleteImage()
    {
        val alertDialog = AlertDialog.Builder(this@Pants)
        ...
        alertDialog.setPositiveButton("Delete") { dialog, which ->
            ...
            if(total==0)
            {
                pants_image?.visibility = View.INVISIBLE
            }
            else
            {
                //show new image
    ...
    fun setNextImage()
    {
        if(total>1)
        {
            ...
            var max = dbHelper.getLastID("Pants")!!
            if (current_id >= max) {
                current_id = 0
            }
            ...
            for (i in current_id + 1..max) {
                if (dbHelper.getBitmapByNameAndID("Pants", i) != null) {
                    val bitmap = Utils.getImage(dbHelper.getBitmapByNameAndID("Pants", i)!!)
    ...
    fun setPreviousImage()
    {
        if(total>1)
        {
            ...
            for (i in current_id - 1 downTo 1) {
                ...
                if (i == 1) {
                    current_id = max
                    val bitmap = Utils.getImage(dbHelper.getBitmapByNameAndID("Pants", current_id)!!)
                    pants_image?.setImageBitmap(bitmap)
                }
            }
        }
    ...
    fun setRandomImage()
    {
        if(total>1)
        {
            ...
            while (dbHelper.getBitmapByNameAndID("Pants", rand) == null || rand == current_id)
            {
    ...
    fun getTotalImages()
    {
        ...
        total = dbHelper.getTotalImages("Pants")!!
    ...
    fun getLastID()
    {
        last_id = dbHelper.getLastID("Pants")!!
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if(resultCode == Activity.RESULT_OK)
        {
            ...
            if(requestCode==IMAGE_CAPTURE_CODE) {
                ...
                pants_image?.setImageURI(image_uri)

                val bitmap = (pants_image?.drawable as BitmapDrawable).bitmap

                DBHelper(applicationContext).addBitmap("Pants", Utils.getBytes(bitmap))
                Toast.makeText(this@Pants, "Saved!", Toast.LENGTH_SHORT).show()
            ...
            if(requestCode==1) {
                ...
                pants_image?.setImageURI(pickedImage)
    ...
    override fun onResume() {
        ...
        //set home
        bottomNavigationView.setSelectedItemId(R.id.navigation_pants)
